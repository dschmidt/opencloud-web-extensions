...
const KdbxwebInit = {
  ...
  argon2(password, salt, memory, iterations, length, parallelism, type, version) {
    ...
    console.log(`[ARGON2] Starting argon2 with params: memory=${memory}, iterations=${iterations}, length=${length}, parallelism=${parallelism}, type=${type}`)
    ...
    return this.loadRuntime(memory).then((runtime) => {
      ...
      return runtime.hash(args).then((hash) => {
        ...
        console.log(`[ARGON2] Hash computed in ${completedTime - loadedTime}ms (total: ${completedTime - startTime}ms)`)
  ...
  loadRuntime(requiredMemory) {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        const hasSIMDSupport = (() => {
          try {
            ...
            return WebAssembly.validate(new Uint8Array([
              0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00, 0x01, 0x05, 0x01, 0x60, 0x01, 0x7b, 0x00, 0x03,
              0x02, 0x01, 0x00, 0x0a, 0x07, 0x01, 0x05, 0x00, 0x20, 0x00, 0x1a, 0x0b
            ]))
          } catch {
            return false
          }
        ...
        console.log(`[ARGON2] Memory calculation: requiredMemory=${requiredMemory}KB, initialMemory=${initialMemory} pages (${initialMemory * WASM_PAGE_SIZE / 1024 / 1024}MB), totalMemory=${totalMemory} pages`)
        ...
        const onMessage = (e) => {
          switch (e.data.op) {
            ...
            case 'postRun':
              ...
              console.log(`[ARGON2] WebAssembly runtime loaded in ${postRunTime - runtimeStartTime}ms`)
                ...
                hash(args) {
                  return new Promise((resolve, reject) => {
                    ...
                    const onHashMessage = (e) => {
                      ...
                      console.log(`[ARGON2] Hash message received in ${hashEndTime - hashStartTime}ms`)
  ...
  calcHash: function (Module, args) {
    const startTime = performance.now()
    console.log('[WORKER] Starting calcHash with params:', {memory: args.memory, iterations: args.iterations, length: args.length, parallelism: args.parallelism, type: args.type})
    ...
    console.log('[WORKER] Module keys:', Object.keys(Module).filter(k => k.includes('argon')))
