<template>
  <div class="group-container" :style="{ marginLeft: (level * 20) + 'px' }">
    ...
    <div v-if="expanded && hasChildren" class="group-content">
      ...
      <!-- Entries Table -->
      <div v-if="group.entries.length > 0" class="entries-table" :style="{ marginLeft: ((level + 1) * 20) + 'px' }">
        ...
        <div v-for="entry in group.entries" :key="entry.uuid.toString()" class="entry-row">
          ...
          <div class="col-password">
            <div class="password-cell">
              <span v-if="!isPasswordVisible(entry.uuid.toString())" class="password-hidden">••••••••</span>
              <span v-else class="password-visible">{{ getFieldText(entry, 'Password') || '' }}</span>
              <button 
                class="password-toggle" 
                @click.stop="togglePassword(entry.uuid.toString())"
                :title="isPasswordVisible(entry.uuid.toString()) ? 'Hide password' : 'Show password'"
              >
